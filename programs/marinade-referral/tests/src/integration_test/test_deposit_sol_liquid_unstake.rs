...
use crate::integration_test::test_state_initialization::{update_referral_execute, get_account};
...
use marinade_referral::constant::DEFAULT_TRANSFER_DURATION;
...
async fn deposit_execute(
    ...
    let accounts = marinade_referral::accounts::Deposit {
        ...
        token_partner_account: partner_token_pubkey,
    };
...
pub async fn do_deposit_sol(
    ...
    deposit_execute(
        ...
        marinade_referral_test_globals.partner_token_pubkey,
        lamports
    ).await;
...
async fn test_deposit_sol_operation_fee() -> anyhow::Result<()> {
    ...
    let referral_msol_balance_before = test
        .get_token_balance_or_zero(&marinade_referral_test_globals.partner_token_pubkey)
        .await;
    ...
    assert!(referral_fee > 0 && referral_fee <= lamports, "Refferal fee cannot be zero and bigger than 100%");
    println!("Basis points fee: {}, deposited lamports: {}, referral fee for deposit operation: {}",
    ...
    assert_eq!(user_sol_balance_after, user_initial_sol_balance - lamports);
    let user_msol_balance_after = test.get_token_balance(&user_msol_account.pubkey).await;
    assert_eq!(user_msol_balance_after, user_msol_balance_before + lamports - referral_fee);
    let referral_msol_balance_after = test
        .get_token_balance_or_zero(&marinade_referral_test_globals.partner_token_pubkey).await;
    assert_eq!(referral_msol_balance_after, referral_msol_balance_before + referral_fee);
